...
nested_flux_kustomization=("./infrastructure/base/sealed-secrets" "./infrastructure/base/external-secrets" "./infrastructure/overlays/development/secrets")
echo "INFO - Validating nested kustomize overlays"
for file in ${nested_flux_kustomization[@]};
  do
    echo "INFO - Validating kustomization ${file/%$kustomize_config}"
    kustomize build "${file/%$kustomize_config}" $kustomize_flags | \
      kubeconform $kubeconform_config
